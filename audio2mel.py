...
class Audio2Mel(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.sample_rate != sample_rate:
            audio = signal.resample(audio, 4 * self.sample_rate)
...
if __name__ == '__main__':
    train_file_list = datasets.get_dataset_filelist()

    print(train_file_list[100])

    train_set = Audio2Mel(train_file_list[0:2], 24000 * 4, 1024, 80, 321, 24000, 0, 10000)

    train_set_instance = train_set[0][0]
    plt.imshow(train_set_instance[0], aspect='auto')
    plt.show()

    print(train_set_instance.shape)
